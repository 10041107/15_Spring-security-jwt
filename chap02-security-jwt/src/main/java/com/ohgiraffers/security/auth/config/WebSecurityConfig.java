...
import com.ohgiraffers.security.auth.filter.CustomAuthFailureHandler;
import com.ohgiraffers.security.auth.filter.CustomAuthSuccessHandler;
...
public class WebSecurityConfig {
    ...
    public WebSecurityCustomizer webSecurityCustomizer() {
        // 요청 리소스가 static resource을 등록하지 않겠다
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.csrf(AbstractHttpConfigurer::disable)
                ...
                // STATELESS: 나는 세션을 만들지 않겠다 = 시큐리티를 통해서 세션을 생성하지 않음
                .formLogin(form -> form.disable()) //시큐리티에서 제공하는 폼을 사용하지 않겠다
                .addFilterBefore(customAuthenticationFilter(), UsernamePasswordAuthenticationFilter.class)
                // customAuthenticationFilter를 만들건데, UsernamePasswordAuthenticationFilter를 대신할거다
    ...
    /**
     * 3. Authentization 의 인증 메소드를 제공하는 매니저로 provider의 인터페이스를 의미한다.
     *
    ...
    /**
     * 8.spring security 기반의 사용자의 정보가 틀릴 경우 수행되는 메서드
